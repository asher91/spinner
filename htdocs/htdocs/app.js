class Spinner extends React.Component {
    ...
    SpinWheelAction(){
        if(this.state.user.balance >= this.state.stake.current){
            ...
            var self = this;
            $.get("http://localhost:8000/htdocs/spin/test/" + this.state.odd + "/" + this.state.stake.current)
            .done(function(data) {
    ...
    LoginAction(e){
        if(this.state.user.username!="" && this.state.user.password!=""){
            var self = this;
            $.post("http://localhost:8000/htdocs/login", this.state.user)
            .done(function(data) {
    ...
    LogoutAction(e){
        var self = this;
        $.post("http://localhost:8000/htdocs/logout", this.state.user)
        .done(function(data) {
    ...
    RegisterAction(e){   
        var self = this;
        $.post("http://localhost:8000/htdocs/register", this.state.user)
        .done(function(data) {
            var response = jQuery.parseJSON(data);
            self.setState({user : response});
            self.CloseOverlay();
...
$(document).ready(function () {
    ...
    $.get("http://localhost:8000/htdocs/game/test")
    .done(function(data) {
        var result = jQuery.parseJSON(data);
        ReactDOM.render(
